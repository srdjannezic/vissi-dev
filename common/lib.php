...
function getSocialLinks($db){
    ...
    foreach($links as $link) : 
        ?>
        <li><a href="<?= $link['url'] ?>" target="_blank"><i class="icon-<?= $link['type']; ?>"></i></a></li>
    <?php endforeach;
}
...
function getImagesFromTable($db,$table,$entity,$id_item=false,$limit=false){

    $query = "SELECT * FROM {$table} WHERE lang = " . LANG_ID . " AND type = 'image'"; 
    if($id_item) $query .= " AND id_item = " . $id_item . ' ORDER BY rank';
