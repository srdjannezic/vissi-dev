...
if($db !== false){
    ...
    if($action == 'duplicate'){
        ...
        $new_id = $id + 1;
        //$data['rank'] = $old_rank;
        foreach($result_exist->fetchAll(PDO::FETCH_ASSOC) as $data){
            $data['id'] = $new_id;
        ...
        if(db_table_exists($db,'pm_'.MODULE.'_file')){
            $query_exist = 'SELECT * FROM pm_'.MODULE.'_file WHERE id_item = '.$id;
            $result_exist = $db->query($query_exist);
            ...
            foreach($result_exist->fetchAll(PDO::FETCH_ASSOC) as $data){
                unset($data['id']);
                $data['id_item'] = $new_id;
                $result_insert = db_prepareInsert($db, 'pm_'.MODULE.'_file', $data); 
                add_item($db, MODULE.'_file', $result_insert, $id_lang);
            }             
        }
    }
}
                                                    ...
                                                    foreach($result as $i => $row){
                                                        ...
                                                        <tr id="item_<?php echo $id ?>">
                                                            ...
                                                            <td class="text-center">
                                                                ...
                                                                if(in_array('duplicate', $permissions) || in_array('all', $permissions)){ ?>
                                                                    <a class="tips" href="index.php?view=list&id=<?php echo $id; ?>&csrf_token=<?php echo $csrf_token; ?>&action=duplicate" title="<?php echo $texts['DUPLICATE']; ?>"><i class="fas fa-fw fa-copy text-success"></i></a>
                                                                    <?php
                                                                }
                                                                if(in_array('delete', $permissions) || in_array('all', $permissions)){ ?>
