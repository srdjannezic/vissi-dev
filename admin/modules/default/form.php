                    ...
                    if($tableName == MODULE || $id_lang == DEFAULT_LANG || $id_lang == 0 || $fields_table['table']['multi'] == 1){
                        ...
                        foreach($fields_table['fields'] as $fieldName => $field){
                            ...
                            elseif(isset($_FILES[$fieldName])){
                                $target_dir = SYSBASE.'medias/'.MODULE.'/big/';

                                $uplField = $_FILES[$fieldName];

                                //var_dump($uplField['name'][0]);

                                $target_file = $target_dir . basename($uplField["name"][0]);

                                if (move_uploaded_file($uplField["tmp_name"][0], $target_file)) {
                                    echo "The file ". basename( $uplField["name"][0]). " has been uploaded.";
                                } else {
                                    echo "Sorry, there was an error uploading your file.";
                                }
                                

                                $value = isset($uplField["name"][0]) ? $uplField["name"][0] : '';
                                $field->setValue($value, $index, $id_lang);
                            }
                        }
                    }
